    ...
    if (mEditable && focused()) {
        if (action == SDL_PRESSED) {
            if (key == SDLK_LEFT) {
				...
				if (mCursorPos > 0)
					mCursorPos -= util::countUTF8segments_reverse(mCursorPos - 1, mValueTemp.c_str());
                ...
                if (mCursorPos < (int) mValueTemp.length())
					mCursorPos += util::countUTF8segments(mValueTemp[mCursorPos]);
                ...
                if (!deleteSelection()) {
                    if (mCursorPos > 0) {
						int count = util::countUTF8segments_reverse(mCursorPos - 1, mValueTemp.c_str());
                        mValueTemp.erase(mValueTemp.begin() + mCursorPos - count, mValueTemp.begin() + mCursorPos);
                        mCursorPos-=count;
                    }
                }
                ...
                if (!deleteSelection()) {
                    if (mCursorPos < (int) mValueTemp.length())
                        mValueTemp.erase(mValueTemp.begin() + mCursorPos, mValueTemp.begin() + mCursorPos + util::countUTF8segments(mValueTemp[mCursorPos]));
                }
...
bool TextBox::keyboardCharacterEvent(const char *utf8) {
    if (mEditable && focused()) {
       // std::ostringstream convert;
        //convert << (char) codepoint;

        deleteSelection();
        mValueTemp.insert(mCursorPos,  utf8 /*convert.str()*/);
		mCursorPos += util::countUTF8segments(utf8[0]);
...
bool TextBox::copySelection() {
    if (mSelectionPos > -1) {
		...
		strncpy_s(c, (end - begin)+1, mValueTemp.c_str()+begin, (end - begin));
		c[(end - begin)] = '\0';
...
float TextBox::cursorIndex2Position(int index, float lastx,
	...
	{
		//char a = mValueTemp[i];
		i += util::countUTF8segments(mValueTemp[i]) - 1;
		cindex++;
	}
...
int TextBox::position2CursorIndex(float posx, float lastx,
    ...
    for (int j = 1; j < size; j++) {
		...
		i += util::countUTF8segments(mValueTemp[i]);
