...
#include "Structs/ConvexHullMesh.hpp"
...
namespace quickhull {
	...
	class QuickHull {
		using vec3 = VectorHull3<FloatType>;
	...
	public:
		...
		ConvexHull<FloatType> getConvexHull(const std::vector<VectorHull3<FloatType>>& pointCloud, bool CCW, bool useOriginalIndices, FloatType eps = Epsilon);
		...
		//      then we generate a new vertex buffer which contains only the vertices that are part of the convex hull.
		//   eps: minimum distance to a plane to consider a point being on positive of it (for a point cloud with scale 1)
		ConvexHull<FloatType> getConvexHull(const VectorHull3<FloatType>* vertexData, size_t vertexCount, bool CCW, bool useOriginalIndices, FloatType eps = Epsilon);
