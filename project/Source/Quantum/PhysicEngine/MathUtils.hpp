...
namespace quickhull {
	...
	namespace mathutils {
		...
		template <typename T>
		inline T getSquaredDistanceBetweenPointAndRay(const VectorHull3<T>& p, const Ray<T>& r) {
			const VectorHull3<T> s = p-r.m_S;
		...
		template <typename T>
		inline T getSignedDistanceToPlane(const VectorHull3<T>& v, const Plane<T>& p) {
			return p.m_N.dotProduct(v) + p.m_D;
		}
		
		template <typename T>
		inline VectorHull3<T> getTriangleNormal(const VectorHull3<T>& a,const VectorHull3<T>& b,const VectorHull3<T>& c) {
